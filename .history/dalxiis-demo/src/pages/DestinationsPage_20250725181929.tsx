    ...
    <div className="min-h-screen bg-gray-50 pt-4">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4 mb-4">
          <h2 className="text-2xl font-bold text-[#1c2c54] mb-2 text-center">
            Activities & Experiences
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            {activities.map((activity, index) => (
              <div key={index} className="text-center p-2 rounded-xl bg-gradient-to-br from-[#f29520] to-[#2f67b5] text-white">
                <div className="text-2xl mb-1">{activity.icon}</div>
                <h3 className="text-base font-semibold mb-1">{activity.name}</h3>
                <p className="text-xs opacity-90 mb-1">{activity.description}</p>
        ...
        {regions.map((region, regionIndex) => (
          <div key={regionIndex} className="mb-6">
            <div className="text-center mb-3">
              <h2 className="text-2xl font-bold text-[#1c2c54] mb-1">{region.name}</h2>
              <p className="text-base text-gray-600 max-w-3xl mx-auto">{region.description}</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {region.destinations.map((destination, destIndex) => (
                <div key={destIndex} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="p-3">
                    <h3 className="text-xl font-bold text-[#1c2c54] mb-1">{destination.name}</h3>
                    <p className="text-gray-600 mb-2 leading-relaxed">{destination.description}</p>
                    ...
                    <div className="mb-2">
                      <h4 className="font-semibold text-[#1c2c54] mb-1">Highlights:</h4>
                      <div className="grid grid-cols-2 gap-1">
                        {destination.highlights.map((highlight, idx) => (
                          <div key={idx} className="flex items-center text-xs text-gray-600">
                            <div className="w-1 h-1 bg-[#f29520] rounded-full mr-1"></div>
                            {highlight}
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="mb-2">
                      <h4 className="font-semibold text-[#1c2c54] mb-1">Activities:</h4>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 bg-[#f29520] text-white px-2 py-1 rounded-full font-semibold hover:bg-[#e08420] transition-colors text-xs"
                      >
                        Plan Visit
                      </button>
                      <button className="flex-1 border-2 border-[#2f67b5] text-[#2f67b5] px-2 py-1 rounded-full font-semibold hover:bg-[#2f67b5] hover:text-white transition-colors text-xs">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4 mb-4">
          ...
          <div className="bg-gradient-to-br from-[#f29520] to-[#2f67b5] rounded-xl p-3 text-white text-center">
            <MapPin className="h-10 w-10 mx-auto mb-1 opacity-80" />
            <h3 className="text-xl font-bold mb-1">Interactive Map Coming Soon</h3>
            ...
            <button
              ...
              className="bg-white text-[#1c2c54] px-4 py-2 rounded-full font-semibold hover:bg-gray-100 transition-colors text-xs"
            >
              Get Directions
            </button>
          </div>
        ...
        {/* CTA Section */}
        <div className="bg-[#1c2c54] text-white rounded-2xl p-4 text-center mb-4">
          <h2 className="text-2xl font-bold mb-2">Ready to Explore Puntland?</h2>
          ...
          <button
            ...
            className="bg-[#f29520] text-white px-4 py-2 rounded-full font-semibold hover:bg-[#e08420] transition-colors text-xs"
          >
