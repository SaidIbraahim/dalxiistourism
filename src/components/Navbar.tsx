...
// Wrap Navbar export with LanguageProvider
...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    // Dedicated Booking entry in the navbar
          ...
          <div className="flex justify-between items-center text-sm">
            ...
            <div className="hidden md:block">
              ...
              <button
                ...
                className="ml-6 px-3 py-1 rounded bg-white/10 text-white border border-white/20 hover:bg-[#f29520] hover:text-white transition-colors text-xs font-semibold"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20">
            ...
            <div className="hidden md:flex items-center space-x-8">
              ...
              <button
                onClick={() => setLanguage(language === 'en' ? 'so' : 'en')}
                className="ml-2 px-3 py-1 rounded bg-[#f29520] text-white border border-[#f29520] hover:bg-[#e08420] transition-colors text-xs font-semibold"
                aria-label="Switch language"
              >
                {language === 'en' ? 'SO' : 'EN'}
              </button>
              ...
              <Link
                to="/admin/login"
                className="px-4 py-2 text-[#1c2c54] border border-[#1c2c54] rounded-lg font-medium hover:bg-[#1c2c54] hover:text-white transition-colors duration-200"
              >
                Admin Login
              </Link>
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-white border-t">
            <div className="px-2 pt-2 pb-3 space-y-1">
              ...
              <button
                onClick={() => setLanguage(language === 'en' ? 'so' : 'en')}
                className="block w-full mt-2 px-3 py-2 rounded-md text-base font-medium bg-[#f29520] text-white text-center"
                aria-label="Switch language"
              >
                {language === 'en' ? 'SO' : 'EN'}
              </button>
              ...
              <Link
                to="/admin/login"
                onClick={toggleMenu}
                className="block px-3 py-2 text-[#1c2c54] border border-[#1c2c54] rounded-md text-center font-medium hover:bg-[#1c2c54] hover:text-white transition-colors"
              >
                Admin Login
              </Link>
