    ...
    <div className="min-h-screen bg-gray-50 pt-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div key={index} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300">
              <div className="p-8">
                <div className="flex items-center mb-6">
                  <div className={`w-16 h-16 ${brandClasses.gradient.secondary} rounded-xl flex items-center justify-center`}>
                    <service.icon className="h-8 w-8 text-white" />
                  </div>
                  <div className="ml-4">
                    <h3 className={`text-2xl font-bold ${brandClasses.text.primary}`}>{service.title}</h3>
                    <p className={`${brandClasses.text.secondary} font-semibold`}>{service.pricing}</p>
                  </div>
                </div>
                ...
                <div className="mb-6">
                  <h4 className={`font-semibold ${brandClasses.text.primary} mb-3`}>Service Features:</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {service.features.map((feature, idx) => (
                      <div key={idx} className="flex items-center text-sm text-gray-600">
                        <div className={`w-1.5 h-1.5 ${brandClasses.bg.secondary} rounded-full mr-3 flex-shrink-0`}></div>
                        {feature}
                      </div>
                    ))}
                  </div>
                ...
                {service.partnerHotels && (
                  <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                    <h4 className={`font-semibold ${brandClasses.text.primary} mb-3`}>Partner Hotels:</h4>
                    {Object.entries(service.partnerHotels).map(([city, hotels]) => (
                      <div key={city} className="mb-2">
                        <span className={`font-medium ${brandClasses.text.tertiary}`}>{city}:</span>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <button
                    ...
                    className={`flex-1 border-2 ${brandClasses.border.tertiary} ${brandClasses.text.tertiary} px-6 py-3 rounded-lg font-semibold ${brandClasses.hover.primary} hover:text-white transition-colors flex items-center justify-center`}
                  >
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
          <h2 className={`text-3xl font-bold ${brandClasses.text.primary} mb-6 text-center`}>Additional Services</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={index} className="text-center p-4 rounded-lg bg-gray-50 hover:bg-[#e0dddf] transition-colors">
                <h3 className={`font-semibold ${brandClasses.text.primary} mb-2`}>{item.title}</h3>
